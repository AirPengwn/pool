...
- **Real (not just testing) caching fix — `data.json` fetch (2026-07-02):** this isn't only a verification annoyance — a genuine site visitor whose browser cached `data.json` on an earlier visit could keep seeing stale numbers on later visits, which defeats the point of a *daily*-updating dashboard. Fixed at the source: `site_src/index.html`'s fetch is now `fetch('data.json', { cache: 'no-store' })`, forcing a real network request every page load instead of allowing the browser's HTTP cache to serve a stale copy. (The outer HTML/JS bundle staying cache-friendly is fine — only the actual data needs to always be fresh.)

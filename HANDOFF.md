...
- **Verify, don't trust the exit code.** A script printing "Appended 1 row" or "updated" is **not** evidence the change landed — three silent-write failures on 2026-08-29 all looked like successes. After writing, **read the data back through a different path** (grep the file, re-open the workbook, query the rendered page). Run **`python check_log.py`** at the end of every logging session; it asserts the log's invariants and exits 1 on any violation. It also runs from the pre-commit hook and in CI, so bad data cannot reach the site. **On a fresh clone, install the hook once: `python tools/install_hooks.py`.** Full rationale in POOL.md → "Silent-failure defences".
